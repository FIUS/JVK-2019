...
public class PhoneBooth extends BasicEntity implements SolidEntity {
     ...
     * If this attribute is false the phonebooth will be solid.
     */
    private boolean isReachable = true;

    /**
     * A checker function that returns true if and only if all requirements
     * to access this phonebooth are met.
     * <p>
    ...
    /**
     * Set a checker function that returns true if and only if all requirements
     * to access this phonebooth are met.
     *
     * @param requirementsChecker the requirementsChecker to set
     */
    ...
    public boolean isCurrentlySolid() {
        if (!this.isReachable) return false;
        // PhoneBooth is reachable, test requirementsChecker
        if (this.requirementsChecker == null) return true;
